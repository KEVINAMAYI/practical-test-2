            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr>
                            <th scope="col" class="px-4 py-4">Contract Number</th>
                            <th scope="col" class="px-4 py-3">Travel Agent</th>
                            <th scope="col" class="px-4 py-3">Start Date</th>
                            <th scope="col" class="px-4 py-3">End Date</th>
                            <th scope="col" class="px-4 py-3">Status</th>
                            <th scope="col" class="px-4 py-3">Exclusive</th>
                        ...
                        <tbody>
                        <tr v-for="contract in contractStore.contracts" :key="contract.id" class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">
                                <p>{{ contract.contract_number }}</p>
                                <p>{{ contract.standard_rate }}</p>
                            </td>
                            <td class="px-4 py-3">
                                <p>{{ contract.travel_agent.user.first_name }}</p>
                                <p>{{ contract.travel_agent.user.last_name }}</p>
                                <p>{{ contract.travel_agent.user.email }}</p>
                            </td>
                            <td class="px-4 py-3">{{ contract.start_date }}</td>
                            <td class="px-4 py-3">{{ contract.end_date  }}</td>
                            <td class="px-4 py-3">{{ contract.status.status }}</td>
                            <td class="px-4 py-3 max-w-[12rem] truncate">{{ contract.exclusive }}</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <button @click="contractStore.deleteContract(contract.id)"
                                    class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100"
                                    type="button">
